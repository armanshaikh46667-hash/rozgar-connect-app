...
async function streamChat({ messages, onDelta, onDone, onError }: {
  ...
  try {
    ...
    if (!resp.ok) { const d = await resp.json().catch(() => ({})); onError(d.error || 'Something went wrong'); return; }
...
const AIChatbot = () => {
  ...
  const send = async () => {
    ...
    await streamChat({
      ...
      onError: (e) => { setMessages((prev) => [...prev, { role: 'assistant', content: `⚠️ ${e}` }]); setLoading(false); },
    });
  };

  return (
    <>
      {/* Floating button with custom robot icon */}
      {!open && (
        <button
          onClick={() => setOpen(true)}
          className="fixed bottom-6 right-4 z-50 w-14 h-14 rounded-full shadow-2xl flex items-center justify-center hover:scale-105 active:scale-95 transition-all ring-4 ring-primary/20 overflow-hidden"
          aria-label="Open AI Chat"
        >
          <img src={chatbotIcon} alt="AI Chat" className="w-full h-full object-cover" />
        </button>
      )}
          ...
          <div className="w-full h-full sm:w-96 sm:h-[520px] sm:rounded-2xl bg-card border border-border shadow-2xl flex flex-col overflow-hidden animate-fade-in">
            <div className="flex items-center justify-between px-4 py-3 bg-primary text-primary-foreground shrink-0">
              <div className="flex items-center gap-2">
                <img src={chatbotIcon} alt="" className="w-8 h-8 rounded-full border-2 border-primary-foreground/30" />
                <div>
                  <p className="text-sm font-bold">RozgarSewa AI</p>
                  <p className="text-[10px] opacity-80">{lang === 'hi' ? 'मदद के लिए पूछें' : 'Ask for help'}</p>
                </div>
              </div>
              <button onClick={() => setOpen(false)} className="p-1 rounded-lg hover:bg-primary-foreground/20 transition-colors"><X size={18} /></button>
            </div>

            <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
              {messages.length === 0 && (
                <div className="text-center py-8">
                  <img src={chatbotIcon} alt="" className="w-16 h-16 mx-auto rounded-full mb-3 shadow-md" />
                  <p className="text-sm font-bold text-foreground">{lang === 'hi' ? 'नमस्ते! 🙏' : 'Hello! 🙏'}</p>
                  <p className="text-xs text-muted-foreground mt-1">{lang === 'hi' ? 'RozgarSewa से जुड़ा कोई भी सवाल पूछें' : 'Ask any question about RozgarSewa'}</p>
              ...
              {messages.map((m, i) => (
                <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] px-3 py-2 rounded-2xl text-sm whitespace-pre-wrap ${m.role === 'user' ? 'bg-primary text-primary-foreground rounded-br-md' : 'bg-secondary text-secondary-foreground rounded-bl-md'}`}>
                    {m.content}
                  </div>
                </div>
              ))}
