...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (mobile.length !== 10) { setError(t('10 अंकों का मोबाइल नंबर डालें')); return; }
    if (pin.length !== 4) { setError(t('4 अंकों का PIN डालें')); return; }
    const result = await login(mobile, pin);
    if (result.success) {
      navigate('/');
    } else {
      setError(result.error || t('लॉगिन विफल'));
    }
  };
    ...
    <div className="min-h-screen bg-background flex flex-col">
      <div className="bg-gradient-to-br from-primary via-primary to-accent-foreground px-6 pt-10 pb-16 text-primary-foreground">
        ...
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-primary-foreground/70 text-xs">{t('अपने अकाउंट में लॉगिन करें')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-lg mx-auto w-full px-4 -mt-8 relative z-10">
        <div className="bg-card rounded-2xl shadow-lg border border-border p-6">
          <h2 className="text-lg font-bold text-foreground mb-1">{t('लॉगिन')}</h2>
          <p className="text-xs text-muted-foreground mb-6">{t('रजिस्टर्ड मोबाइल नंबर और Secret PIN डालें')}</p>

          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-foreground mb-1.5 block">{t('मोबाइल नंबर')}</label>
              <div className="relative">
                <Phone size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <input type="tel" inputMode="numeric" maxLength={10} value={mobile}
                  onChange={(e) => setMobile(e.target.value.replace(/\D/g, ''))}
                  placeholder={t('10 अंकों का मोबाइल नंबर')}
            ...
            <div>
              <label className="text-xs font-semibold text-foreground mb-1.5 block">{t('Secret PIN')}</label>
              <div className="relative">
                <Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
                <input type="password" inputMode="numeric" maxLength={4} value={pin}
                  onChange={(e) => setPin(e.target.value.replace(/\D/g, ''))}
                  placeholder={t('4 अंकों का PIN')}
                  className="w-full pl-10 pr-4 py-3 rounded-xl border border-input bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              <LogIn size={18} />
              {loading ? t('लॉगिन हो रहा है...') : t('लॉगिन करें')}
            </button>
          </form>

          <p className="text-xs text-muted-foreground text-center mt-6">
            {t('अकाउंट नहीं है?')}{' '}
            <button onClick={() => navigate('/business-register')} className="text-primary font-semibold">{t('रजिस्टर करें')}</button>
          </p>
        </div>
      </div>
    </div>
