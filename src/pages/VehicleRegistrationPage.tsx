    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="bg-gradient-to-br from-primary via-primary to-accent-foreground px-6 pt-8 pb-6 text-primary-foreground">
          <div className="max-w-lg mx-auto"><h1 className="text-xl font-extrabold flex items-center gap-2"><Car size={22} /> {t('वाहन सेवाएँ', lang)}</h1></div>
        </div>
    ...
    <div className="min-h-screen bg-background">
      <div className="bg-gradient-to-br from-primary via-primary to-accent-foreground px-6 pt-8 pb-6 text-primary-foreground">
        <div className="max-w-5xl mx-auto">
          ...
          <h1 className="text-xl font-extrabold flex items-center gap-2"><Car size={22} /> {t('वाहन सेवाएँ रजिस्ट्रेशन', lang)}</h1>
          <p className="text-primary-foreground/80 text-xs mt-1">{t('ड्राइवर, मैकेनिक, ट्रांसपोर्ट आदि', lang)}</p>
        </div>
      </div>
        ...
        <div className="lg:grid lg:grid-cols-3 lg:gap-6">
          <div className="lg:col-span-2 bg-card rounded-2xl shadow-lg border border-border p-5">
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <input type="text" value={serviceCharge} onChange={(e) => setServiceCharge(e.target.value)} placeholder={t('जैसे: ₹15/km', lang)} className={`${inputClass} mt-2`} maxLength={50} />
              </div>
              <div>
          ...
          <div className="hidden lg:block space-y-4">
            <div className="bg-card rounded-2xl border border-border p-5">
              <h3 className="text-sm font-bold text-foreground mb-3">🚗 {lang === 'hi' ? 'वाहन सेवा टिप्स' : 'Vehicle Service Tips'}</h3>
              <ul className="space-y-2 text-xs text-muted-foreground">
                <li>✅ {lang === 'hi' ? 'सही वाहन श्रेणी चुनें' : 'Choose correct vehicle category'}</li>
                <li>📍 {lang === 'hi' ? 'GPS से ग्राहक आसानी से मिलेंगे' : 'GPS helps customers find you'}</li>
                <li>💰 {lang === 'hi' ? 'किलोमीटर रेट लिखें' : 'Mention per km rate'}</li>
              </ul>
            </div>
          </div>
        </div>
