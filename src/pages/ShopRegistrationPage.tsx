...
const SHOP_CATEGORIES = [
  ...
  'Clothing Store', 'Mobile Shop', 'Stationery Shop',
];
...
const ShopRegistrationPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!ownerName || !shopName || !mobile || !finalCategory || !village) return;
    ...
    const { error } = await supabase.from('local_businesses').insert({
      ...
      photo: photo || null, lat: lat || null, lng: lng || null, pin,
    });
    setSubmitting(false);
    if (error) { toast.error(t('कुछ गलत हो गया, दोबारा कोशिश करें', lang)); return; }
    setSuccess(true);
    toast.success(t('दुकान सफलतापूर्वक रजिस्टर हो गई! 🎉', lang));
  };
    ...
    return (
      <div className="min-h-screen bg-background">
        <div className="bg-gradient-to-br from-primary via-primary to-accent-foreground px-6 pt-8 pb-6 text-primary-foreground">
          <div className="max-w-lg mx-auto"><h1 className="text-xl font-extrabold flex items-center gap-2"><Store size={22} /> {t('दुकान रजिस्ट्रेशन', lang)}</h1></div>
        </div>
        <div className="max-w-lg mx-auto px-4 -mt-4 relative z-10">
          <div className="bg-card rounded-2xl shadow-lg p-8 text-center animate-fade-in">
            ...
            <h2 className="text-xl font-bold text-foreground mb-2">{t('दुकान रजिस्टर हो गई!', lang)}</h2>
            <p className="text-sm text-muted-foreground mb-6">{t('आपकी दुकान अब लाइव है।', lang)}</p>
            <button onClick={() => navigate('/businesses')} className="w-full bg-primary text-primary-foreground py-3 rounded-xl text-sm font-bold">{t('दुकानें देखें', lang)}</button>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-background">
      <div className="bg-gradient-to-br from-primary via-primary to-accent-foreground px-6 pt-8 pb-6 text-primary-foreground">
        <div className="max-w-5xl mx-auto">
          ...
          <h1 className="text-xl font-extrabold flex items-center gap-2"><Store size={22} /> {t('दुकान रजिस्ट्रेशन', lang)}</h1>
          <p className="text-primary-foreground/80 text-xs mt-1">{t('अपनी दुकान रजिस्टर करें', lang)}</p>
        </div>
      </div>
          ...
          <div className="lg:col-span-2 bg-card rounded-2xl shadow-lg border border-border p-5">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="flex flex-col items-center gap-2">
                <label className="text-xs font-bold text-muted-foreground block">{t('दुकान फोटो (वैकल्पिक)', lang)}</label>
              ...
              <div><label className="text-xs font-bold text-muted-foreground mb-1 block">{t('मालिक का नाम', lang)} *</label><input type="text" value={ownerName} onChange={e => setOwnerName(e.target.value)} placeholder={t('मालिक का नाम लिखें', lang)} className={inputClass} required maxLength={100} /></div>
              <div><label className="text-xs font-bold text-muted-foreground mb-1 block">{t('दुकान का नाम', lang)} *</label><input type="text" value={shopName} onChange={e => setShopName(e.target.value)} placeholder={t('दुकान का नाम लिखें', lang)} className={inputClass} required maxLength={100} /></div>
              ...
              <div>
                ...
                {!useCustom ? (
                  ...
                  <>
                    <input type="text" value={customCategory} onChange={(e) => setCustomCategory(e.target.value)} placeholder={t('अपनी दुकान श्रेणी लिखें', lang)} className={inputClass} required={useCustom} maxLength={50} />
              ...
              <div><label className="text-xs font-bold text-muted-foreground mb-1 block">{t('विवरण (वैकल्पिक)', lang)}</label><textarea value={description} onChange={e => setDescription(e.target.value)} placeholder={t('दुकान के बारे में संक्षिप्त विवरण', lang)} className={`${inputClass} resize-none h-20`} maxLength={300} /></div>
              ...
              <button type="submit" disabled={submitting} className="w-full bg-primary text-primary-foreground py-3.5 rounded-xl text-sm font-bold shadow-lg active:scale-[0.97] transition-transform disabled:opacity-60">
                {submitting ? t('रजिस्टर हो रहा है...', lang) : t('दुकान रजिस्टर करें', lang)}
              </button>
            </form>
          </div>
          <div className="hidden lg:block space-y-4">
            <div className="bg-card rounded-2xl border border-border p-5">
              <h3 className="text-sm font-bold text-foreground mb-3">🏪 {lang === 'hi' ? 'दुकान टिप्स' : 'Shop Tips'}</h3>
              <ul className="space-y-2 text-xs text-muted-foreground">
                <li>✅ {lang === 'hi' ? 'दुकान का सही नाम डालें' : 'Enter correct shop name'}</li>
                <li>📍 {lang === 'hi' ? 'GPS लोकेशन से ग्राहक आसानी से मिलेंगे' : 'GPS helps customers find you'}</li>
                <li>📸 {lang === 'hi' ? 'दुकान की फोटो विश्वास बढ़ाती है' : 'Shop photo increases trust'}</li>
              </ul>
            </div>
          </div>
        </div>
